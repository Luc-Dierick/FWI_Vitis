...
//    std::string binaryFile = argv[1]; //"/shares/bulk/ldierick/workspace/fwi_system/Hardware/FullW.xclbin";
//    cl_int err;
//    cl::Context context;
...
//    vector<float,aligned_allocator<float>> vectorDot(COL);
//    vector<complex_float,aligned_allocator<complex_float> > kappaTimesResSWD(ROW,0);
//    vector<complex_float,aligned_allocator<complex_float> > kappaTimesResHWD(ROW,0);
//
//
//   for(i = 0; i <COL; i++){
//	   vectorDot[i] = (float)(i);
...
//    }
//
//    std::cout << " UPDATE TEST 1" << (matchu ? "PASSED" : "FAILED") << std::endl;
//
//    // Allocate Buffer in Global Memory
//        // Buffers are allocated using CL_MEM_USE_HOST_PTR for efficient memory and Device-to-host communication
